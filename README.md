...
1. Положительные тесты:  
    ...
    Файл находится в тестовой среде по пути: /tests/data.txt
    ...
    ![Результаты](./tests/results/tests%201+.png)  
   ...
   - tests/
     ...
     - sub_folder_1/
      ...
      ![Результаты](tests/results/tests%202+.png)
...
2. Негативные тесты:
   ...
   Ожидаемый результат: encryptData() -> false и сообщение об ошибке.
    ...
    Ожидаемый результат: Операция должна пройти без ошибок encryptData() -> true.
     ...
     ![Результаты](tests/results/tests%205-.png)  
    ...
    Ожидаемый результат: Тест проверяет, что система не падает при обработке гигантских входных данных для ключа; успешное выполнение encryptData() -> true означает устойчивость к таким нагрузкам.
     ...
     ![Результаты](tests/results/tests%206-.png)

    8) testNoEncryptLabel()
    Суть: Проверяем что не можем шифровать папку по ярлыку
    Ожидаемый результат: Папка по ярлыку не шифруется. В задании указано что защищаем только данные которые находятся по пути.
    encryptData() -> true, проверяем что данные в папке по ярлыку не зашифрованы, это критерий прохождения тест-кейса.

    ![Результаты](tests/results/tests%207-.png)
